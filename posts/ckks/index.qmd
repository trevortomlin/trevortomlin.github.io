...
```{python}
def coordinate_wise_randomized_rounding(coordinates):
    r = coordinates - np.floor(coordinates)
    f = np.array([np.random.choice([c, c-1], 1) for c in r]).reshape(-1)
...
def cwrr(coords):
    decimals = coords - np.floor(coords)
    f = np.array([np.random.choice([a-1, a], 1) for a in decimals]).reshape(-1)
    return np.array([int(coord) for coord in coords + f])
